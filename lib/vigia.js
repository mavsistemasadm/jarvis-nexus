...
async function avaliar() {
  ...
  /* As regras rodam em paralelo, então a ordem de chegada varia entre
     execuções. Sem um critério de desempate, dois alertas de mesma
     severidade trocam de lugar e o resumo sai diferente a cada rodada —
     era o que fazia o dedupe nunca reconhecer o próprio registro. */
  alertas.sort((x, y) => ORDEM[x.severidade] - ORDEM[y.severidade]
    || String(x.chave).localeCompare(String(y.chave)));
...
/* Impressão digital da SITUAÇÃO, não do texto.
   O texto carrega coisas que mudam sozinhas ("1d" vira "2d" na virada do
   dia) e não carrega ordem estável. A assinatura usa chave + título, que é
   onde moram as contagens e os totais: muda quando o quadro muda, e só. */
function assinatura({ alertas }) {
  return alertas
    .map(a => `${a.chave}|${a.titulo}`)
    .sort()
    .join('\n');
}

module.exports = { avaliar, resumir, resumirWhatsapp, assinatura, SEV };
